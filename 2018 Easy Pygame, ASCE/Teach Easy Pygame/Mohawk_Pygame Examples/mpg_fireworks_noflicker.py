...
This example does a manual flip to reduce flicker. Notice that doing fewer
flips also speeds up the animation!
...
def rocket():
  ...
  while y>EXPLODE_HEIGHT:
    mpg.draw_ellipse(round(WIDTH/2), y, 10, 30, flip=False)
    mpg.flip()
    mpg.pause(0.005)
...
def explosion(x, y, red, green, blue):
  ...
  while offSet < MAX_SIZE: 
    mpg.draw_ellipse(x+offSet, y+offSet, PARTICLE_SIZE, PARTICLE_SIZE, flip=False)
    mpg.draw_ellipse(x+offSet, y-offSet, PARTICLE_SIZE, PARTICLE_SIZE, flip=False)
    mpg.draw_ellipse(x-offSet, y+offSet, PARTICLE_SIZE, PARTICLE_SIZE, flip=False)
    mpg.draw_ellipse(x-offSet, y-offSet, PARTICLE_SIZE, PARTICLE_SIZE, flip=False)
    mpg.draw_ellipse(x, y+offSet, PARTICLE_SIZE, PARTICLE_SIZE, flip=False)
    mpg.draw_ellipse(x, y-offSet, PARTICLE_SIZE, PARTICLE_SIZE, flip=False)
    mpg.draw_ellipse(x+offSet, y, PARTICLE_SIZE, PARTICLE_SIZE, flip=False)
    mpg.draw_ellipse(x-offSet, y, PARTICLE_SIZE, PARTICLE_SIZE, flip=False)
    mpg.flip()
    mpg.pause(0.005)
    mpg.clear(flip=False)
